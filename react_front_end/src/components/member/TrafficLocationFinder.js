...
const TrafficLocationFinder = ({ onTrafficIncidentData, onLocationFinderOpen }) => {//sends incident data and boolean for opening/closing the location finder
	...
	useEffect(() => {
		...
		if (isMounted) {
			...
			console.log('[TrafficLocationFinder - useEffect] mounted');
	...
	function saveLocation(location_name){
		...
		axios.post('/api/save_traffic_location', data, {
			headers: {
				'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
			}
		}).then(response =>{
			...
			<OffCanvasMenu >
				<div id="location_finder" className="z-index-2100 bg-fafafa bl1-ccc pt-70l-110m-50s" style={{height:"2000px", overflow:"hidden"}} ref={locationFinderRef}>
					<div className="clearfix p-10">
						...
						<Link to="#" className="button icon close-mobile-nav text-center right" onClick={toggleLocationFinder}  onTouchEnd={toggleLocationFinder} ref={closeLocationFinderRef}>
			        	...
			        	{trafficLocations.map((location, i) => (
							...
							<span key={'delete_'+i} className="right">
								<span className="opacity-50">
									<img src={delete_icon} className="" width="17" alt="delete location"/>
								</span>
							</span>
							) : (
							<span key={'delete_'+i} className="right">
...
export default TrafficLocationFinder;
